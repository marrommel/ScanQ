                    ...
                    TextFormField(
                        ...
                        decoration: InputDecoration(
                            ...
                            labelText: AppLocalizations.of(context)!
                                .targetLanguage,
                            icon: getFlagByLanguageCode(
                                widget.category.categoryLanguage
                            )
                        ),
                        validator: (String? value) {
                          if (value == null || value.trim().isEmpty) {
                        ...
                        onPressed: () async {
                          if (_editVocabularyFormKey.currentState!.validate()) {
                            if (vocabLocal.isNotEmpty &&
                                ...
                                (vocabLocal != widget.vocabulary.vocLocal ||
                                    vocabForeign != widget.vocabulary.vocForeign)) {
                              final Database db = Provider.of<Database>(context);
                              if (vocabLocal != widget.vocabulary.vocLocal) {
                                await db.changeVocLocal(
                                    vocabLocal, widget.vocabulary.id);
                              }
                              if (vocabForeign != widget.vocabulary.vocForeign) {
                                await db.changeVocForeign(
                                    vocabForeign, widget.vocabulary.id);
                              }
                            }
                          }
                        },
